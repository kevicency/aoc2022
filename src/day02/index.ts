...
const normalize = (char: string) => toCharCode(char) - toCharCode(char >= "X" ? "X" : "A")
...
const score = ([op, me]: number[]) => me + 1 + 3 * (me === next(op) ? 2 : +(me === op))

const part1 = (rawInput: string) => pipe(map(score), sum)(parseInput(rawInput))

const choosePlay = ([op, me]: number[]) => [op, [prev(op), op, next(op)][me]]

const part2 = (rawInput: string) => pipe(map(choosePlay), map(score), sum)(parseInput(rawInput))
